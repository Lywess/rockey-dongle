...
# endif
     ...
     },
    {
     ...
     TLS1_RFC_ECDHE_WITH_SM4_GCM_SM3,
     TLS1_CK_ECDHE_WITH_SM4_GCM_SM3,
     SSL_kSM2DH,
     SSL_aSM2DSA,
     SSL_SM4GCM,
     SSL_AEAD,
...
# endif
    ...
    if (SSL_IS_TLS13(s) || SSL_IS_DTLS13(s)) {
...
# endif
     ...
     */
#ifndef OPENSSL_NO_CNSM
    if ((s->version >= TLS1_VERSION || s->version == SM1_1_VERSION)
#else
...
EVP_PKEY *ssl_generate_pkey_group(SSL *s, uint16_t id)
{
    ...
    if (SM1_1_VERSION != s->version) {
        ...
        pctx = EVP_PKEY_CTX_new(s->cert->pkeys[SSL_PKEY_SM2_ENC].privatekey, NULL);
    }
    if (pctx == NULL) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_SSL_GENERATE_PKEY_GROUP,
...
EVP_PKEY *ssl_generate_param_group_sm2(void)
{
    EVP_PKEY_CTX *pctx = NULL;
    EVP_PKEY *pkey = NULL;
...
int ssl_derive_SM2(SSL *s, EVP_PKEY *privkey, EVP_PKEY *pubkey,  int gensecret)
{
    ...
    srvr_pub_pkey = X509_get0_pubkey(s->session->peer_enc);
    ...
    if (local_e_sm4)
        ...
        EVP_PKEY_CTX_set_app_data(pctx, (void *)0);
    ...
    if (gensecret) {
        ...
        if (SSL_IS_TLS13(s)) {
            ...
            if(local_evp_ptr && local_e_sm4){
                ...
                if(set_engine_flag == 1){
                    ENGINE_unset_tass_flags(local_e_sm4, TASS_FLAG_PRE_MASTER_KEY_CIPHER);
                    set_engine_flag = 0;
                }
            }
            else
