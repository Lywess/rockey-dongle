...
BIO *PKCS7_dataInit(PKCS7 *p7, BIO *bio, int no_hash)       
{
    ...
    //0：计算Z值，填充到bio中，标准签名
    //1:不进行计算Z值操作，直接返回一个先创建的空bio，外部进行hash计算，配合PKCS7_dataFinal传入32字节的hash值，然后进行裸签； 
    if(no_hash == 1)
	{    
    ...
    int have_z = 0;
    unsigned char digest[1024*10+SM3_DIGEST_LENGTH];
    size_t dgst_len = 32;
    X509_ALGOR *alg = NULL;
    const EVP_MD *md = NULL;
    if(md_sk){
        alg = sk_X509_ALGOR_value(md_sk, 0);
        if(alg == NULL){
        	printf("sk_x509_ALGOR_value fail!\n");
        	goto err;
        }
        ...
        if(EVP_MD_type(md) == NID_sm3){
            ...
            if (!ECDSA_sm2_get_Z((const EC_KEY *)EVP_PKEY_get0_EC_KEY(pkey), NULL, NULL, 0, digest, &dgst_len)){
                goto err;
            }
            else{
                have_z = 1;
...
BIO *PKCS7_dataDecode(PKCS7 *p7, EVP_PKEY *pkey, BIO *in_bio, X509 *pcert)
{
    ...
    case NID_pkcs7_sm2_signedAndEnveloped:
        ...
        if (evp_cipher == NULL) {
#ifndef OPENSSL_NO_CNSM  //for compatible SM4 OID (defined in GMT 0006-2014)
...
int PKCS7_dataFinal(PKCS7 *p7, BIO *bio, int no_hash)
{
    int ret = 0;
    int i, j;
    BIO *btmp;
    ...
    EVP_MD_CTX *mdc = NULL, *ctx_tmp = NULL;
    ...
    if (si_sk != NULL) {
        for (i = 0; i < sk_PKCS7_SIGNER_INFO_num(si_sk); i++) {
            ...
            if(no_hash == 1)
            {
                    ...
                    if(32 != BIO_read(bio, dgst, 32)){
                            printf("read from bio fail!\n");
                            goto err;
                    }
                    
                    if (!PKCS7_is_detached(p7)) {
                            if(32 != BIO_write(bio, dgst, 32)){
                                    printf("write from bio fail!\n");
                                    goto err;
                            }
                    }
                    ...
                    if (EVP_PKEY_sign(pkctx, abuf, &abuflen, dgst, 32) <= 0)
                    {
                            if(pkctx != NULL) 
							{
                                    EVP_PKEY_CTX_free(pkctx);    
    ...
    if (!PKCS7_is_detached(p7)) {
        ...
        if (!(os->flags & ASN1_STRING_FLAG_NDEF)) {
            ...
            //read Z (writed in function PKCS7_dataInit) from BIO, so there is only original data(to be signed data) in BIO
            //modified by ysc at 20201226
            if( no_hash == 0 && OBJ_obj2nid(p7->type) == NID_pkcs7_sm2_signed )
            {
                //First read Z
                if( 32 != BIO_read(btmp, tmp_buf, 32 ) )
                {
                    goto err;
                }

                //Second read original data
                contlen = BIO_get_mem_data(btmp, NULL);

                cont = OPENSSL_malloc(contlen);
                if( NULL == cont )
                {
                    goto err;
                }

                if( contlen != BIO_read(btmp, cont, contlen) )
                {
                    OPENSSL_free(cont);
                    goto err;
                }
                BIO_clear_flags(btmp, BIO_FLAGS_MEM_RDONLY);
            }
            else
            {
                          ...
                          X509 *x509, int no_hash)
{
    ...
    EVP_PKEY *pkey = NULL;
    	...
    	if(EVP_PKEY_verify_init(pkctx) <= 0)
    	    goto err;
    	i = EVP_PKEY_verify(pkctx, si->enc_digest->data, si->enc_digest->length, dgst, 32);
 		gerr:
            if(pkey != NULL) 
                    EVP_PKEY_free(pkey);
 			if(pkctx != NULL) 
